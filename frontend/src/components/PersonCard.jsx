import { Building2, Hash, Network, ExternalLink } from 'lucide-react';
...
export default function PersonCard({
  ...
  const personTitle = person.title || 'Professional';
      ...
      <div>
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-slate-600 mb-1.5 truncate">{personTitle}</p>
        ...
        {showSharedTopics && sharedTopics.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1">
            {sharedTopics.slice(0, 3).map((topic) => (
              <span
                key={topic.id}
                className="text-xs bg-primary-50 text-primary-700 px-2 py-0.5 rounded-full flex items-center"
              >
                <Hash className="w-3 h-3 mr-0.5 text-primary-500" />
                {topic.name}
              </span>
            ))}
            {sharedTopics.length > 3 && (
              <span className="text-xs text-slate-500">+{sharedTopics.length - 3} more</span>
            )}
          </div>
        )}

        {personBio && !showSharedTopics && (
          <p className="text-sm text-slate-600 mt-3 line-clamp-2">{personBio}</p>
        )}
      </div>

      {/* Action footer: Profile link + LinkedIn Connect + Network Path */}
      <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          ...
          {personId !== currentUserId && (
            <Link
              to={`/network/${currentUserId}/${personId}`}
              title="View connection graph path"
              className="p-1.5 hover:bg-slate-100 text-slate-500 hover:text-primary-600 rounded-lg transition-colors"
            >
